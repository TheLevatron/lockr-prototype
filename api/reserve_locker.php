...
try {
    if (USE_JSON_STORAGE) {
        ...
        $all_reservations = $jsonDB->findAll('reservations.json', 'user_id', $user_id);
        $active_count = 0;
        ...
        $all_years = $jsonDB->findAll('academic_years.json', 'is_active', 1);
        $academic_year = !empty($all_years) ? $all_years[0] : null;
        $academic_year_id = $academic_year ? $academic_year['year_id'] : null;
        $validity_date = $academic_year ? $academic_year['end_date'] : null;
        
        // Insert reservation
        $reservation = [
            'user_id' => $user_id,
            'locker_id' => $locker_id,
            'referral_slip_path' => $referral_slip_path,
        ...
        $jsonDB->insert('reservations.json', $reservation);
